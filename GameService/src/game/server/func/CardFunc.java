...
//        }
    ...
    public static Card getCardFromCardString(String strCard) {
        try {
            int num = Integer.parseInt(strCard.split("_")[0].trim());
            CardType type = CardType.valueOf(Integer.parseInt(strCard.split("_")[1].trim()));
    ...
    public static int getCoinUpdate(List<Card> list, int betCoin, int stealVal, int limit) {
        int coin = 0;
        if (stealVal != 0) {
            coin += (list.size() * 2 + (stealVal - 1) * 2) * betCoin;
        } else {
            coin += 10 * betCoin * CardFunc.getCountCardNumTwo(list);
            coin += 10 * betCoin * CardFunc.getCountFourOfKind(list);
            coin += list.size() * betCoin;
            if (list.size() == limit) {
                coin += limit * betCoin;
            }
        }
    ...
    public static int getCalcCoinEndTurn(List<Card> listAtk, List<Card> listDef, int betcoin) {
        if (isFourOfAKind(listDef) || isLinkNPairs(listDef, 3) || isLinkNPairs(listDef, 4)) {
            return listAtk.size() * betcoin * 10;
        }
        return 0;
    }
